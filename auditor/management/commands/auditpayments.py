...
from auditor.management.commands.payaudits import REQUESTER_GRACE_PERIOD
...
class Command(BaseCommand):
    ...
    def __notify_requester(self, requester, requester_audit):
        ...
        subject = "Fair Work: Mechanical Turk bonuses pending for $%.2f" % self.__get_underpayment(requester_audit)
    ...
    def __audit_list_message(self, assignments_to_bonus, is_worker, is_html):
        ...
        for hit_type in hit_types:
            ...
            if underpayment is None:
                ...
                summary = "HIT Type {hittype:s} originally paid ${payment:.2f} per task. Median estimated time across workers was {estimated:s}, for an estimated rate of ${paymentrate:.2f}/hr. Bonus ${bonus:f} for each of {num_assignments:d} assignments to bring the payment to a suggested ${paymentrevised:f} each. Total: ${totalbonus:.2f} bonus. [FREEZE LINK]".format(hittype = hit_type.id, payment = hit_type.payment, estimated = time_nomicroseconds, paymentrate = hittype_assignments[0].estimated_rate, bonus = bonus, num_assignments = len(hittype_assignments), paymentrevised = paymentrevised, totalbonus = self.__get_underpayment(hittype_assignments))
            s += summary
